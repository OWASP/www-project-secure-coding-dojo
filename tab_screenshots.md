---
title: Screenshots
layout:  null
tab: true
order: 1
tags: secure-coding-dojo
---

### Training Portal Front Page
![Alt text](assets/images/frontpage.png?raw=true "Secure Coding Dojo Training Portal Front Page")

### Leaderboard
![Alt text](assets/images/leaderboard.png?raw=true "Leaderboard")

### Secure Coding Practices (Code Blocks)
![Alt text](assets/images/codeblocks.png?raw=true "Code Blocks")

### Training modules
![Alt text](assets/images/lessons.png?raw=true "Training modules")

### Code Review training module
Check it out from Github as a static page: [Security Code Review 101 (Github Static Site)](https://trendmicro.github.io/SecureCodingDojo/codereview101/)
![Alt text](assets/images/codereview.png?raw=true "Code review training")

### Common Software flaws training module
![Alt text](assets/images/challenges.png?raw=true "Common flaws training")

### Detailed description for a challenge
![Alt text](assets/images/challengedetails.png?raw=true "Challenge details")

### Common Software flaws - Insecure.Inc Vulnerable site
![Alt text](assets/images/insecureinc.png?raw=true "Insecure.Inc Vulnerable site")

### XSS challenge
![Alt text](assets/images/xss.png?raw=true "XSS challenge")

### Buffer overflow challenge
![Alt text](assets/images/bufferoverflow.png?raw=true "Buffer overflow")
